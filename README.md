...
3. **Étape 3 :** Refactoring itératif :
   - Commit 4 → Typage fort (`domain/`)  
   - Commit 5 → Extraction `discountService`  
   - Commit 6 → Extraction `loyaltyService`  
   - Commit 7 → Construction `CustomerReport` (`reportBuilder`)  
   - Commit 8 → Formatage et génération (`reportFormatter`, `generateReport`)  
